...
const faq = () => {
    ...
    return (
      <>
        <div className="flex max-w-screen-2xl mx-auto justify-center my-16 rounded-3xl bg-gray-100">
          <div className="pb-10">
        ...
        {accordions.map((PerAccordion, idx) => (
          <div key={idx} className="mb-4 px-10 border-b-2 border-[#eeeeee] py-2">
            ...
            <div className={`grid text-start overflow-hidden text-gray-500 transition-all duration-300 ease-in-out ${isOpen === idx ? 'grid-rows-[1fr] pb-3 opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
              <div className="overflow-hidden text-lg">{PerAccordion.description}</div>
            </div>
          </div>
        ))}
      </div>
